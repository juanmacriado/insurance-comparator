            ...
            <div className="bg-gradient-to-r from-xeoris-blue to-gray-900 p-8 text-white relative border-b border-gray-800">
                <div className="flex flex-col md:flex-row justify-between items-center gap-6 relative z-10">
                    <div className="text-center md:text-left">
                        ...
                        <p className="text-gray-400 text-xs uppercase tracking-[0.2em] font-bold">Resumen de coberturas por Xeoris Global Risk</p>
                    </div>
                </div>
            </div>
                    ...
                    <thead>
                        <tr className="bg-gray-900/80 border-b border-gray-800">
                            ...
                            {report.policyNames.map((name, i) => (
                                <th key={i} className="p-6 font-black text-white border-l border-gray-800">
                                ...
                                <tr key={idx} className={cn(
                                    ...
                                    <td className="p-6">
                                        <div className="flex items-center gap-3">
                                            {!isPremiumRow && <Shield className="w-4 h-4 text-gray-700 group-hover:text-xeoris-yellow transition-colors" />}
                                            <span className={cn(
                                                "font-bold text-sm block",
                                                isPremiumRow ? "text-xeoris-yellow" : "text-gray-300"
                                            )}>
                                                {item.category}
                                            </span>
                                        </div>
                                    ...
                                    {item.policies.map((p, pIdx) => (
                                        <td key={pIdx} className="p-6 border-l border-gray-800 relative">
                                            {p.isWinner && (
                                                <div className="absolute top-4 right-4 bg-xeoris-yellow/20 p-1 rounded-full border border-xeoris-yellow/30">
                                                    <Check className="w-3 h-3 text-xeoris-yellow" />
                                                </div>
                                            )}

                                            <p className="text-xs text-gray-400 mb-4 leading-relaxed line-clamp-4 hover:line-clamp-none transition-all">
                                                {p.details}
                                            </p>
                                            ...
                                            <div className="space-y-2">
                                                {p.amount !== "N/A" && (
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[8px] font-black text-gray-600 uppercase tracking-tighter">Suma:</span>
                                                        <span className="text-xeoris-yellow font-black text-xs bg-xeoris-yellow/5 px-2 py-0.5 rounded border border-xeoris-yellow/10">
                                                            {p.amount}
                                                        </span>
                                                    </div>
                                                )}
                                                {p.deductible !== "N/A" && (
                                                    <div className="flex items-center gap-2 opacity-60">
                                                        <span className="text-[8px] font-black text-gray-600 uppercase tracking-tighter">Franq:</span>
                                                        <span className="text-[10px] font-medium text-gray-400">{p.deductible}</span>
                                                    </div>
                                                )}
                                                {p.scope !== "No especificado" && p.scope !== "No analizado" && (
                                                    <div className="flex items-center gap-2 mt-2 pt-2 border-t border-gray-800/50">
                                                        ...
                                                        <span className="text-[9px] text-gray-500 italic truncate" title={p.scope}>{p.scope}</span>
                                                    </div>
                                                )}
                                            </div>
                                        </td>
